...
profiles, read [Auth Clients](/start/auth-clients) after this.

<AccordionGroup>
  <Accordion title="1. Install gog" icon="download">
    Start with Homebrew for the fastest path, then confirm the binary with `gog --version`.
  </Accordion>
  <Accordion title="2. Create or reuse a Google Cloud project" icon="cloud">
    `gog auth setup` can guide the common `gcloud` path, including API enablement and browser handoff.
  </Accordion>
  <Accordion title="3. Store credentials and authorize an account" icon="key">
    Save the Desktop OAuth client JSON, run `gog auth add`, and verify the resulting token health with `gog auth doctor --check`.
  </Accordion>
  <Accordion title="4. Run real commands" icon="terminal">
    Once auth is working, move straight into Gmail, Calendar, Drive, Docs, Sheets, and Slides commands from the same CLI surface.
  </Accordion>
</AccordionGroup>

<CardGroup cols={2}>
  <Card title="Install options" icon="download" href="/start/install">
    Compare Homebrew, Docker, Windows, release archives, and source builds.
  </Card>
  <Card title="Command index" icon="terminal" href="/commands">
    Browse the generated command surface after your first successful login.
  </Card>
</CardGroup>
...
[Auth Clients](/start/auth-clients).
...
See [Workspace Admin](/guides/workspace/workspace-admin) for service-account setup, generated
...
<CardGroup cols={2}>
  <Card title="Auth Clients" icon="user-shield" href="/start/auth-clients">
    Named clients, service accounts, ADC, and Workspace delegation.
  </Card>
  <Card title="Gmail Workflows" icon="envelope" href="/guides/gmail/gmail-workflows">
    Practical read and automation examples once auth is in place.
  </Card>
  <Card title="Drive Audits" icon="folders" href="/guides/drive-files/drive-audits">
    Safe inspection patterns for large folders and sharing reviews.
  </Card>
  <Card title="Workspace Admin" icon="users-gear" href="/guides/workspace/workspace-admin">
    Domain-wide delegation, user management, org units, and groups.
  </Card>
</CardGroup>

- [Auth Clients](/start/auth-clients) — named clients, service accounts, ADC,
  Workspace domain-wide delegation, OIDC subject migration.
- [Workspace Admin](/guides/workspace/workspace-admin) — create, inspect, suspend, delete,
  and list Workspace users, organizational units, and groups.
- [Safety Profiles](/reference/safety-profiles) — runtime allow/deny lists and baked
  agent-safe binaries.
- [Gmail Workflows](/guides/gmail/gmail-workflows) and [Drive Audits](/guides/drive-files/drive-audits) for
